...
<template>
    ...
    <div v-if="page.props.flash?.success" class="grow flex items-center justify-center text-center">
        <div class="mx-auto max-w-sm sm:max-w-md">
            ...
            <Link href="/site/contact" class="inline-block bg-primary-600 hover:bg-primary-700 text-white font-medium px-6 py-3 rounded-lg transition-colors">Send another message</Link>
        </div>
    </div>

    <!-- Contact form -->
    <div v-else class="grow flex items-center justify-center py-4">
        <div class="overflow-hidden rounded-2xl shadow-lg dark:shadow-gray-900/50 bg-gray-50 dark:bg-gray-800 w-full max-w-[960px]">
                ...
                <div class="w-full md:w-2/3">
                    <div class="p-6 lg:p-8">
                        <div class="text-center mb-6">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">Fill out the form below and we will get back to you</p>
                        </div>
                        ...
                        <form @submit.prevent="submit">
                            ...
                            <div class="flex items-center justify-between gap-4 flex-wrap">
                                <VueTurnstile
                                    ...
                                    theme="auto"
                                />
                                <button type="submit" class="login-btn text-white px-6 py-2.5 rounded-lg cursor-pointer" :disabled="form.processing || !form['ContactForm[turnstileToken]']">Submit</button>
                            </div>
                            <p v-if="fieldError('turnstileToken')" class="text-red-600 dark:text-red-400 text-sm mt-1">{{ fieldError('turnstileToken') }}</p>
                        </form>
                    </div>
                </div>
